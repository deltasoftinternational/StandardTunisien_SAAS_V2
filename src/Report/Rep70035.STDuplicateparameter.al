report 70035 "STDuplicate parameter"
{
    ...
    dataset
    {
        ...
        {
            DataItemTableView = SORTING("Payment Class", Line);
            dataitem("ST Autorisation Etapes"; "ST Autorisation Etapes")
            {


                trigger OnAfterGetRecord()
                var
                    RecAutorisationEtapes: Record "ST Autorisation Etapes";
                begin
                    RecAutorisationEtapes.Copy("ST Autorisation Etapes");
                    RecAutorisationEtapes.Validate("STPayment Class", NewName);
                    RecAutorisationEtapes.Validate(STLine, "Payment Step".Line);
                    RecAutorisationEtapes.Insert();
                end;

                trigger OnPreDataItem()
                begin
                    SetRange("STPayment Class", PaymentClass.Code);
                    SetRange(STLine, "Payment Step".Line);
                end;
            }
            trigger OnAfterGetRecord()
            var
